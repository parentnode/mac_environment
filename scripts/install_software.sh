...
if [ "$install_software" = "Y" ]; then
    ...
    # if [ "$install_webserver_conf" = "Y" ]; then
        ...
        sudo port -N install mariadb-10.5-server
	    ...
	    if [ -n "$mariadb_installed_specific" ]; then
			...
			echo "Preparing runtime environment for Database"
            ...
            copyFile "/srv/tools/conf/my.cnf" "/opt/local/etc/mariadb-10.5/my.cnf" 
            if [ $(fileExist "/opt/local/var/db/mariadb-10.5/mysql") = "false" ]; then 
                ...
                outputHandler "comment" "Database already installed"
            fi
            command "sudo port load mariadb-10.5-server"
        else 
            outputHandler "comment" "Mariadb 10.5 not installed" "Please try installing Mariadb 10.5 again at a later time" "Run this install script again afterwards"
        fi
